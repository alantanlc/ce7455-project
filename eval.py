...
def main(args):
  ...
  for train_name, pred_answers in zip(training_split, pred_answers_list):
    if len(gold_answers) != len(pred_answers):
      raise Exception("The prediction file seems incomplete or formatted incorrectly.")

    accuracy = accuracy_score(gold_answers, pred_answers)
    results['accuracy_' + train_name] = accuracy
    y.append(accuracy)

  results["AUC"] = auc(x, y) / x_diff  # normalized area under (learning) curve)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Evaluate WinoGrande predictions')
  # Required parameters
  ...
  parser.add_argument('--metrics_output_file', type=str, help='Location of output metrics file', default='metrics.json')
